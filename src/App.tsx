...
function App() {
  return (
    <div className='relative w-screen min-h-screen bg-orange-50'>
      ...
      <div className='w-full mt-48 justify-center flex'><button className='w-96 bg-orange-700 border-orange-700 text-white hover:text-zinc-800 hover:border hover:border-solid hover:bg-white rounded-full px-8 py-4'>Help Me</button></div>
            ...
            {data.map(d=>{
              return (
                <tr className='border-orange-50'>
                  ...
                  <td className='flex w-96 flex-wrap p-4 gap-1'>{d.tags.map(t=>{
                    return (
                      <MiniTag>{t.name}</MiniTag>
                    )
                  })}</td>
                  <td className='text-xs p-4'>This resource helped <span className='text-lg bg-orange-200 rounded-full px-2 py-1'>{d.helped_num}</span>people</td>
                </tr>
              )
            })}
          </tbody>
